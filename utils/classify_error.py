...
def classify_error(error_message: str) -> str:
    ...
    # DuckDB: "Referenced column X not found in FROM clause" or "Table X does not have a column named Y"
    if re.search(
        r"\bcolumn\b.*\b(not found|does not exist|not exist)\b"
        r"|\breference[d]?\s+column\b.*\bnot\s+found\b"
        r"|\bdoes\s+not\s+have\s+a\s+column\s+named\b"
        r"|\bno\s+column\s+named\b",
        msg_lower,
    ):
